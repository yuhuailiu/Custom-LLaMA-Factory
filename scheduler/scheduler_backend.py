...
class JobScheduler:
    ...
    @staticmethod
    def _parse_yaml_command(task_str: str) -> tuple[str, str, str, str]:
        """Parse incoming task string for command, yaml path, devices, and eval template.

        Accepted formats:
        - "path/to/config.yaml" -> assumes command 'train', default devices, eval enabled with PT template
        - "path/to/config.yaml@0,1,2,3" -> assumes command 'train', specified devices, eval enabled with PT template
        - "path/to/config.yaml@0,1,2,3#evalSFT" -> assumes command 'train', specified devices, eval enabled with SFT template
        - "path/to/config.yaml@0,1,2,3#evalPT" -> assumes command 'train', specified devices, eval enabled with PT template
        - "train:/abs/path.yaml" -> default devices, eval enabled with PT template
        - "train:/abs/path.yaml@0,1,2,3" -> specified devices, eval enabled with PT template
        - "train:/abs/path.yaml@0,1,2,3#evalSFT" -> specified devices, eval enabled with SFT template
        - "train:/abs/path.yaml@0,1,2,3#evalPT" -> specified devices, eval enabled with PT template
        - "eval:/abs/path.yaml@0,1" -> specified devices, eval enabled with PT template
        """
        task = task_str.strip()
        devices = ""  # Empty means use default
        eval_template = "NONE"  # Default to no evaluation
        
        # First, check if this is a prefixed command (train:/path.yaml or eval:/path.yaml)
        if ':' in task and not task.startswith('/') and not task.startswith('./') and not task.startswith('../'):
            # This looks like a prefixed command
            prefix, rest = task.split(':', 1)
            prefix = prefix.strip().lower()
            if prefix in {'train', 'eval'}:
                # Now parse the rest for devices and eval template
                yaml_path = rest.strip()
                
                # Check for eval template flag with #
                if '#' in yaml_path:
                    yaml_path, eval_flag = yaml_path.rsplit('#', 1)
                    eval_flag = eval_flag.strip()
                    if eval_flag == 'evalSFT':
                        eval_template = "SFT"
                    elif eval_flag == 'evalPT':
                        eval_template = "PT"
                
                # Check for devices specification with @
                if '@' in yaml_path:
                    yaml_path, devices = yaml_path.rsplit('@', 1)
                    devices = devices.strip()
                
                return prefix, yaml_path, devices, eval_template
        
        # Handle non-prefixed YAML files
        # Check for eval template flag with #
        if '#' in task:
        ...
        if task.lower().endswith('.yaml'):
            return 'train', task, devices, eval_template
        
        raise ValueError("Unrecognized YAML task format. Use '/path/to.yaml[@devices][#evalSFT|#evalPT]' or 'train:/path.yaml[@devices][#evalSFT|#evalPT]' or 'eval:/abs/config.yaml[@devices]'.")
        ...
        while self.is_running:
            try:
                ...
                try:
                    ...
                    eval_template = "NONE" # type: ignore[var-annotated]
                    ...
                    try:
                        ...
                        command, yaml_path, devices, eval_template = self._parse_yaml_command(str(script_path).strip())
                        is_yaml_task = True
                    ...
                    if ran_yaml_task and ran_yaml_path:
                        ...
                        if is_train_run and ran_yaml_out_dir:
                            ...
                            # Run evaluation only if explicitly requested
                            if eval_template == "NONE":
                                logging.info(f"Evaluation not requested for {ran_yaml_path} (no eval flag specified)")
                            elif eval_template == "PT": # Use PT template
                                try:
                                    eval_yaml_path = self._gen_eval_yaml_blocking(ran_yaml_path, template="PT")
                                    if eval_yaml_path:
    ...
    def _gen_eval_yaml_blocking(self, train_yaml_path: str, template: str = "PT") -> str:
        """Generate eval YAML using our helper script and return its absolute path."""
        rc, out, err = self._run_python_in_env([GEN_EVAL_SCRIPT, '--train_yaml', train_yaml_path, "--eval_template", EVAL_TEMPLATE_PT if template == "PT" else EVAL_TEMPLATE_SFT])
        if rc != 0:
    ...
    def _collect_eval_results_blocking(self) -> None:
        ...
        rc, out, err = self._run_python_in_env([COLLECT_EVAL_SCRIPT, '--root', EVAL_RESULTS_ROOT, "--categorized"])
        if rc != 0:
                        ...
                        # --- Added terminate command ---
                            ...
                            if task.endswith('.sh') and os.path.exists(task):
                                ...
                                try:
                                    ...
                                    command, yaml_path, devices, eval_template = self._parse_yaml_command(task)
                            ...
                            if not accept and not reason:
                                reason = "Unknown task format. Submit a .sh, .yaml[@devices][#evalSFT|#evalPT], or 'train:/path.yaml[@devices][#evalSFT|#evalPT]'/'eval:/path.yaml[@devices]'."
...
def submit_job(task_str):
    # Accept .sh, .yaml, or prefixed command like 'train:/path.yaml', with optional @devices and #evalSFT|#evalPT
    devices = ""
    eval_template = "NONE"  # Default to no evaluation
    original_task = task_str
    
    # Extract eval template flag if specified with #
    ...
    if task_str.endswith('.sh'):
        ...
        if not os.path.exists(abs_path):
            ...
            return
        payload = abs_path + (f"@{devices}" if devices else "") + (f"#evalSFT" if eval_template == "SFT" else (f"#evalPT" if eval_template == "PT" else ""))
    elif ':' in task_str:
        ...
        if not os.path.exists(abs_path) or not abs_path.endswith('.yaml'):
            ...
            return
        payload = f"{prefix}:{abs_path}" + (f"@{devices}" if devices else "") + (f"#evalSFT" if eval_template == "SFT" else (f"#evalPT" if eval_template == "PT" else ""))
    else:
        print("❌ Error: Unsupported task. Provide a .sh, .yaml[@devices][#evalSFT|#evalPT], or 'train:/path.yaml[@devices][#evalSFT|#evalPT]'/'eval:/path.yaml[@devices]'.")
        return
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.connect((HOST, PORT))
...
if __name__ == '__main__':
    ...
    submit_parser.add_argument('task', type=str, help="Path to .sh/.yaml[@devices][#evalSFT|#evalPT], or 'train:/abs/config.yaml[@devices][#evalSFT|#evalPT]'/'eval:/abs/config.yaml[@devices]'. Use @0,1,2,3 to specify device IDs. Use #evalSFT or #evalPT to enable evaluation (no evaluation by default).")

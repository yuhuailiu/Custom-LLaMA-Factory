...
if [ "$#" -ge 1 ]; then
  ...
  CONFIGS=(
    "/data2/lyh/Custom-LLaMA-Factory/train_yaml/stage1_pretrain/qwen_3b_pretrain_8k_len_lora128_ep2.yaml"
    "/data2/lyh/Custom-LLaMA-Factory/train_yaml/stage1_pretrain/qwen_3b_pretrain_8k_len.yaml"
  )
...
for YAML_FILE in "${CONFIGS[@]}"; do
  ...
  $PYTHON_CMD "$MERGE_SCRIPT" --yaml "$YAML_FILE" --clean_checkpoints
